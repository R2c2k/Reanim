...
local function saveFavorites()
    ...
    if success then
        local saveSuccess, errorMessage = pcall(function()
            writefile("favorite_animations.json", encodedFavorites) -- Changed to writefile
        end)
        ...
        while fakeAnimRunning do
            ...
            for ii, frame in pairs(keyframes) do
                ...
                if ii < #keyframes then
                    ...
                    -- Then wait the appropriate amount of time before next frame
                    task.wait(waitTime)
                else
            ...
            -- After playing through all frames once, wait a tiny bit before looping
            task.wait(1 / 30)
        end
    end)
end
